<template>
  <div>
    ...
    <div
      v-if="top10Loading && top10Data.length === 0"
      class="loading-state"
    >
      <div class="spinner" />
    ...
    <div
      v-else-if="top10Error && top10Data.length === 0"
      class="error-state"
    >
    ...
    <template v-else>
      <div
        v-if="top10Loading"
        class="top10-refreshing"
      >
        <div class="top10-refreshing-dot" /> 갱신 중...
      </div>
      <div
        v-if="top10Error && top10Data.length > 0"
        class="top10-error-inline"
      >
        ⚠️ {{ top10Error }}
      </div>
      ...
      <div class="top10-table-wrap">
        <table class="top10-table">
          ...
          <tbody>
            <tr
              v-for="stock in top10Data"
              :key="stock.symbol"
            >
              <td class="col-rank">
                <span :class="['rank-badge', rankClass(stock.rank)]">{{ stock.rank }}</span>
                <span
                  v-if="stock.rankChange > 0"
                  class="rank-chg rank-up"
        ...
        >
          ...
          <div class="t10c-mcap">
            ...
            <span
              v-if="stock.volume"
              class="t10c-vol"
            > · 거래량 {{ formatVolume(stock.volume) }}</span>
          </div>
        </div>
      </div>
    </template>
