        ...
        >
          ...
          <template v-if="hoveredSegment">
            <text
              ...
              fill="var(--text-muted)"
              style="pointer-events:none"
            >{{ hoveredSegment.name.length > 8 ? hoveredSegment.name.slice(0,7) + '…' : hoveredSegment.name }}</text>
            <text
              y="-3"
              text-anchor="middle"
              font-size="14"
              ...
              fill="var(--accent)"
              style="pointer-events:none"
            >{{ (hoveredSegment.pct * 100).toFixed(1) }}%</text>
            <text
              y="14"
              text-anchor="middle"
              font-size="7.5"
...
export default {
  ...
  props: {
    ...
    hoveredSegId: { type: [Number, String], default: null },
    hoveredSegment: { type: Object, default: null },
