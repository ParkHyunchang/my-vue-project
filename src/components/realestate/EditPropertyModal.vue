    ...
    <div v-if="show" class="rem-overlay" data-lenis-prevent @click.self="close">
      <div class="rem-box">
        ...
        <div class="rem-fixed">
          ...
          <div class="rem-fixed-hint">소재·지번·명칭·면적은 변경할 수 없어요 (바꾸려면 삭제 후 재등록).</div>
        </div>
        ...
        <!-- 월세 (아파트 월세만) -->
        <div v-if="!isLand && h?.dealType === 'MONTHLY'" class="rem-row">
          <label>월세 (만원)</label>
        ...
        <template v-if="isLand">
          <div class="rem-grid2">
            <div class="rem-row">
              <label>지목</label>
              <input v-model="form.jimok" type="text" list="edit-land-jimok" placeholder="예: 대" autocomplete="off" />
            ...
            <div class="rem-row">
              <label>용도지역</label>
              <input v-model="form.useZone" type="text" list="edit-land-zone" placeholder="예: 계획관리" autocomplete="off" />
          ...
          <div class="rem-row">
            ...
            <div class="rem-official">
              <input v-model.number="form.officialPricePerM2" type="number" min="0" placeholder="원/㎡" class="rem-official-input" />
              <input v-model.number="form.officialPriceYear" type="number" min="2000" :max="thisYear" placeholder="연도" class="rem-official-year" />
              <button type="button" class="rem-official-btn"
                      :disabled="!h?.bdongCode || officialLoading" @click="refetchOfficial">
                {{ officialLoading ? '조회중…' : '최신 재조회' }}
              </button>
            </div>
            <span v-if="officialMsg" class="rem-hint">{{ officialMsg }}</span>
            <span v-else-if="!h?.bdongCode" class="rem-hint">읍면동·지번 정보가 없어 자동 조회 불가 (직접 입력)</span>
          </div>
        </template>
        ...
        <div class="rem-actions">
          ...
          <button class="rem-submit" :disabled="saving" @click="submit">
            {{ saving ? '저장중…' : '수정' }}
          </button>
        </div>
      </div>
...
.rem-close { border: none; background: none; font-size: 18px; cursor: pointer; color: var(--text-muted); }
...
.rem-fixed-row { display: flex; align-items: center; gap: 8px; }
.rem-fixed-badge {
  font-size: 11px; font-weight: 700; padding: 2px 9px; border-radius: 10px;
  background: #e0e7ff; color: #3730a3;
}
.rem-fixed-name { font-size: 15px; font-weight: 700; color: var(--text-primary); }
.rem-fixed-sub { font-size: 12px; color: var(--text-muted); margin-top: 6px; }
.rem-fixed-hint { font-size: 11px; color: var(--text-muted); opacity: 0.8; margin-top: 8px; }
...
.rem-official { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; }
.rem-official-input { flex: 1 1 120px; min-width: 0; }
.rem-official-year { flex: 0 0 84px; }
.rem-official-btn {
  padding: 9px 12px; border: 1px solid #6366f1; border-radius: 7px;
...
@media (max-width: 480px) {
  ...
  .rem-official-input, .rem-official-year { flex: 1 1 100%; }
}
</style>
